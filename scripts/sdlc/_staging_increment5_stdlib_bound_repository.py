#!/usr/bin/env python3
"""Materialize the stdlib-only, exact-repository profile validator.
...
SOURCE_HEAD = "52a4031148aa3526ea19c65408fa6d2c99e643fd"
CONTRACT_DIGEST = (
...
MANIFEST = ROOT / "increment5a-stdlib-bound-repository-manifest.json"
...
def replace_section(
    ...
    *,
    ...
    if text.find(start_marker, start + 1) >= 0:
        raise RuntimeError(f"{label}: non-unique start marker")
...
def update_tests() -> None:
    ...
    text = replace_once(
        ...
        '        "validation_code_origin": "EXACT_TRACKED_EXECUTABLE_STDLIB_ONLY",\n'
        '        "validation_data_origin": "EXACT_REVIEWED_GIT_BLOBS",\n',
        label="receipt v4 expectation",
    )
    text = text.replace(
        '"CACHE_FREE_EXACT_GIT_ARCHIVE"',
    ...
    start = "def test_nonisolated_execution_rejects_before_pythonpath_dependency_import(\n"
    end = "def test_isolated_validator_rejects_noncanonical_and_duplicate_json() -> None:\n"
    replacement = r'''def test_nonisolated_execution_rejects_before_dependency_import(
    tmp_path: Path,
) -> None:
...
'''
    text = replace_section(
        ...
        replacement,
        label="validator boundary test block",
    )
    if "_CONTRACT_RELATIVE_PATH" not in text:
        insertion = (
...
def update_docs() -> None:
    ...
    section = '''### Profile-validation authority boundary
...
'''
    decision = replace_section(
        decision,
        "### Profile-validation authority boundary\n",
        "These profiles are not production deployment profiles.",
        section,
        label="decision validator boundary",
    )
    DECISION.write_text(decision, encoding="utf-8")

    evaluation = EVALUATION.read_text(encoding="utf-8")
    old_start = "Cross-Epoch pooling is prohibited. A missing or mismatched Epoch is\n"
    old_end = "Superseded Epoch Runs remain retained."
    paragraph = '''Cross-Epoch pooling is prohibited. A missing or mismatched Epoch is
...
`NOT_EVALUATED`. Superseded Epoch Runs remain retained.'''
    evaluation = replace_section(
        evaluation,
        old_start,
        old_end,
        paragraph,
        label="evaluation receipt boundary",
    )
    EVALUATION.write_text(evaluation, encoding="utf-8")

    operations = OPERATIONS.read_text(encoding="utf-8")
    profile_section = '''## Profiles and validation
...
'''
    operations = replace_section(
        ...
        "## Epoch admission\n",
        profile_section,
        label="operations validator boundary",
    )
...
def main() -> None:
    ...
    changed = sorted(
        ...
        for path in (VALIDATOR, TESTS, DECISION, EVALUATION, OPERATIONS)
    )
    manifest = {
        "changed_paths": changed,
        "contract_digest_unchanged": CONTRACT_DIGEST,
        "evaluation_plan_digest_unchanged": PLAN_DIGEST,
        "python_dependency_boundary": {
            "environment_packages_used": False,
            "repository_python_imports_used": False,
            "runtime": "STDLIB_ONLY",
        },
        "repository_boundary": {
            "explicit_git_dir": True,
            "explicit_index_file": True,
            "explicit_work_tree": True,
            "reject_assume_unchanged": True,
            "reject_skip_worktree": True,
        },
        "reviewed_data_boundary": {
            "exact_git_blobs": sorted(_ for _ in []),
            "executable_blob_content": False,
        },
        "schema_version": "newsroom.increment5a.stdlib-bound-repository.v1",
        "source_head": SOURCE_HEAD,
    }
    manifest["reviewed_data_boundary"]["exact_git_blobs"] = [
        "increment5a_retrieval_contract_v1.json",
        "fixture_structural_schema",
        "qualification_structural_schema",
        "fixture_reviewed_binding_schema",
        "qualification_reviewed_binding_schema",
    ]
    MANIFEST.write_text(
