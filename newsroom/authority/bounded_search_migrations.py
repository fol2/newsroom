...
BOUNDED_SEARCH_MIGRATION_STATEMENTS: tuple[str, ...] = (
    ...
    f"""CREATE TABLE search_requests(
        ...
        query_privacy TEXT NOT NULL,
        max_provider_calls INTEGER NOT NULL CHECK(max_provider_calls BETWEEN 1 AND 9007199254740991),
        max_results INTEGER NOT NULL CHECK(max_results BETWEEN 1 AND 9007199254740991),
        max_gross_cost_microunits INTEGER NOT NULL CHECK(max_gross_cost_microunits>=0),
        max_elapsed_seconds INTEGER NOT NULL CHECK(max_elapsed_seconds BETWEEN 1 AND 9007199254740991),
    ...
    f"""CREATE TABLE search_attempts(
        ...
        attempt_ordinal INTEGER NOT NULL CHECK(attempt_ordinal BETWEEN 1 AND 9007199254740991),
        variant_ordinal INTEGER NOT NULL CHECK(variant_ordinal BETWEEN 1 AND 9007199254740991),
        language_ordinal INTEGER NOT NULL CHECK(language_ordinal BETWEEN 1 AND 9007199254740991),
        page_number INTEGER NOT NULL CHECK(page_number BETWEEN 1 AND 9007199254740991),
        retry_ordinal INTEGER NOT NULL CHECK(retry_ordinal BETWEEN 0 AND 9007199254740991),
        branch_ordinal INTEGER NOT NULL CHECK(branch_ordinal BETWEEN 0 AND 9007199254740991),
    ...
    f"""CREATE TABLE search_result_references(
        ...
        request_id TEXT NOT NULL REFERENCES search_requests(request_id),
        request_digest TEXT NOT NULL CHECK({_D.format('request_digest')}),
        rank INTEGER NOT NULL CHECK(rank BETWEEN 1 AND 9007199254740991),
        page_number INTEGER NOT NULL CHECK(page_number BETWEEN 1 AND 9007199254740991),
    ...
    f"""CREATE TABLE search_budget_ledger(
        ...
        entry_kind TEXT NOT NULL CHECK(entry_kind IN ('PURPOSE_ALLOCATION','REQUEST_ALLOCATION','ATTEMPT_ALLOCATION','OUTCOME','RESULT_REFERENCE','REVIEW_DECISION','DOWNSTREAM_WORK')),
        purpose_id TEXT REFERENCES search_purposes(purpose_id),
        ...
        request_id TEXT REFERENCES search_requests(request_id),
        attempt_id TEXT REFERENCES search_attempts(attempt_id),
        work_reference_digest TEXT CHECK(work_reference_digest IS NULL OR {_D.format('work_reference_digest')}),
        gross_cost_microunits INTEGER NOT NULL CHECK(gross_cost_microunits>=0),
        cumulative_provider_calls INTEGER NOT NULL CHECK(cumulative_provider_calls BETWEEN 0 AND 9007199254740991),
        ...
        cumulative_downstream_work_items INTEGER NOT NULL CHECK(cumulative_downstream_work_items BETWEEN 0 AND 9007199254740991),
        ledger_digest TEXT NOT NULL UNIQUE CHECK({_D.format('ledger_digest')}),
        recorded_at TEXT NOT NULL,
        UNIQUE(request_id,work_reference_digest),
        CHECK(
            (entry_kind='PURPOSE_ALLOCATION' AND purpose_id IS NOT NULL
             AND outcome_id IS NULL AND result_reference_id IS NULL
             AND review_decision_id IS NULL AND request_id IS NULL
            ...
            OR
            (entry_kind='REQUEST_ALLOCATION' AND purpose_id IS NOT NULL
             AND outcome_id IS NULL AND result_reference_id IS NULL
             AND review_decision_id IS NULL AND request_id IS NOT NULL
             AND attempt_id IS NULL AND work_reference_digest IS NULL
             AND gross_cost_microunits=0 AND cumulative_provider_calls=0
             AND cumulative_results=0 AND cumulative_gross_cost_microunits=0
             AND cumulative_downstream_work_items=0)
            OR
            (entry_kind='ATTEMPT_ALLOCATION' AND purpose_id IS NULL AND outcome_id IS NULL
             AND result_reference_id IS NULL AND review_decision_id IS NULL
             ...
             AND cumulative_downstream_work_items=0)
            OR
            (entry_kind='OUTCOME' AND purpose_id IS NULL AND outcome_id IS NOT NULL AND result_reference_id IS NULL
             AND review_decision_id IS NULL
             AND request_id IS NOT NULL
             AND attempt_id IS NOT NULL AND work_reference_digest IS NULL
             AND cumulative_provider_calls>=1 AND cumulative_downstream_work_items=0)
            OR
            (entry_kind='RESULT_REFERENCE' AND purpose_id IS NULL AND outcome_id IS NOT NULL
             ...
             AND cumulative_downstream_work_items=0)
            OR
            (entry_kind='REVIEW_DECISION' AND purpose_id IS NULL AND outcome_id IS NULL
             ...
             AND cumulative_downstream_work_items=0)
            OR
            (entry_kind='DOWNSTREAM_WORK' AND purpose_id IS NULL AND outcome_id IS NULL
             AND result_reference_id IS NULL AND review_decision_id IS NOT NULL
             AND request_id IS NOT NULL
    ...
    "CREATE UNIQUE INDEX search_budget_purpose_identity ON search_budget_ledger(purpose_id) WHERE entry_kind='PURPOSE_ALLOCATION'",
    "CREATE UNIQUE INDEX search_budget_request_identity ON search_budget_ledger(request_id) WHERE entry_kind='REQUEST_ALLOCATION'",
    "CREATE UNIQUE INDEX search_budget_attempt_identity ON search_budget_ledger(attempt_id) WHERE entry_kind='ATTEMPT_ALLOCATION'",
    "CREATE UNIQUE INDEX search_budget_outcome_attempt ON search_budget_ledger(attempt_id) WHERE entry_kind='OUTCOME'",
    ...
    """CREATE TRIGGER insert_once_search_budget_ledger BEFORE INSERT ON search_budget_ledger
       WHEN EXISTS(SELECT 1 FROM search_budget_ledger WHERE ledger_entry_id=NEW.ledger_entry_id OR ledger_digest=NEW.ledger_digest OR (entry_kind='PURPOSE_ALLOCATION' AND NEW.entry_kind='PURPOSE_ALLOCATION' AND purpose_id=NEW.purpose_id) OR (entry_kind='REQUEST_ALLOCATION' AND NEW.entry_kind='REQUEST_ALLOCATION' AND request_id=NEW.request_id) OR (entry_kind='OUTCOME' AND NEW.entry_kind='OUTCOME' AND outcome_id=NEW.outcome_id) OR result_reference_id=NEW.result_reference_id OR (entry_kind='REVIEW_DECISION' AND NEW.entry_kind='REVIEW_DECISION' AND review_decision_id=NEW.review_decision_id) OR (entry_kind='DOWNSTREAM_WORK' AND NEW.entry_kind='DOWNSTREAM_WORK' AND review_decision_id=NEW.review_decision_id) OR (entry_kind='ATTEMPT_ALLOCATION' AND NEW.entry_kind='ATTEMPT_ALLOCATION' AND attempt_id=NEW.attempt_id) OR (entry_kind='OUTCOME' AND NEW.entry_kind='OUTCOME' AND attempt_id=NEW.attempt_id) OR (request_id=NEW.request_id AND work_reference_digest=NEW.work_reference_digest AND NEW.work_reference_digest IS NOT NULL) OR (request_id=NEW.request_id AND entry_kind='OUTCOME' AND NEW.entry_kind='OUTCOME' AND cumulative_provider_calls=NEW.cumulative_provider_calls) OR (request_id=NEW.request_id AND entry_kind='DOWNSTREAM_WORK' AND NEW.entry_kind='DOWNSTREAM_WORK' AND cumulative_downstream_work_items=NEW.cumulative_downstream_work_items))
       BEGIN SELECT RAISE(ABORT,'bounded Search identity already retained'); END""",
    *tuple(
