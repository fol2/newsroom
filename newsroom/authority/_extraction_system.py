...
from newsroom.authority._extraction_facade import GovernedExtraction
from newsroom.authority._extraction_store import _ExtractionAuthorityStore
...
class GovernedExtractionAuthoritySystem:
    __slots__ = ("sources", "checks", "discovery", "extraction", "__close")
...
def open_governed_extraction_authority_system(
    *,
    ...
    discovery_registry, discovery_schemas = merge_discovery_signal_lead_registries(
    ...
    merged_registry, merged_schemas = merge_extraction_authority_registries(
        command_registry=discovery_registry,
        payload_schemas=discovery_schemas,
    )
    ...
    try:
        ...
        return GovernedExtractionAuthoritySystem(
            ...
            extraction=GovernedExtraction(
                ...
                register_run=extraction_boundary.register_run,
                record_attempt=extraction_boundary.record_attempt,
                retain_output=extraction_boundary.retain_output,
                retain_proposal_set=extraction_boundary.retain_proposal_set,
                contract=extraction_boundary.contract,
                current_contract=extraction_boundary.current_contract,
                run=extraction_boundary.run,
                attempt=extraction_boundary.attempt,
                attempts=extraction_boundary.attempts,
                output=extraction_boundary.output,
                proposal_set=extraction_boundary.proposal_set,
                replay=extraction_boundary.replay,
            ),
            close=close,
        )
    except Exception:
