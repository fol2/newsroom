...
class _Increment4Neo4jBoundary:
    ...
    def _require_source_snapshot(self, request: Increment4Neo4jBuildRequest) -> int:
        source_watermark = self._store.latest_projection_source_ledger_seq()
        if request.snapshot.through_ledger_seq != source_watermark:
            raise ProjectionStateError(
                "Increment 4 snapshot must bind the exact current source watermark"
            )
    ...
    def build_and_promote(
        ...
        with self._operation_lock:
            ...
            source_watermark = self._require_source_snapshot(request)
            ...
            if metadata.generation.state is ProjectionGenerationState.ACTIVE:
                deleted_target = self._adapter.cleanup_generation(
                ...
                if state_digest != validation.projection_state_digest:
                    raise Neo4jIdentityConflict(
                        "Increment 4 replay graph differs from retained validation"
                    )
