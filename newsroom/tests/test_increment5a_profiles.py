...
def test_profile_api_has_no_caller_supplied_contract_authority() -> None:
    for function in (
        build_fixture_replay_manifest,
        build_qualification_manifest,
        validate_profile_manifest,
...
def test_repository_contract_must_bind_exact_profile_schema_identities(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    wrong_schema_identities = MappingProxyType(
        {
            RetrievalProfileKind.FIXTURE_REPLAY.value: _DIGEST_A,
            RetrievalProfileKind.PRODUCTION_SHAPED_QUALIFICATION.value: _DIGEST_B,
        }
    )
    unreviewed = replace(
        INCREMENT_5A_CONTRACT,
        profile_schema_digests=wrong_schema_identities,
    )
    monkeypatch.setattr(profiles, "load_repository_contract", lambda: unreviewed)

    with pytest.raises(
        Increment5ProfileError,
        match="profile schema identities differ",
    ):
        build_fixture_replay_manifest(
            fixture_id="integrated-fixture-v3",
            fixture_manifest_digest=_DIGEST_A,
...
def test_fixture_replay_profile_is_hermetic_and_never_qualification_evidence() -> None:
    ...
    assert validated.profile_schema_digest == FIXTURE_REPLAY_PROFILE_SCHEMA_DIGEST
...
def test_production_shaped_qualification_is_actual_service_but_not_production() -> None:
    ...
    assert validated.profile_schema_digest == QUALIFICATION_PROFILE_SCHEMA_DIGEST
...
def test_profile_contract_and_component_identity_are_exact_schema_constants() -> None:
    ...
    with pytest.raises(Increment5ProfileError, match="schema validation failed"):
        validate_profile_manifest(tampered)
