...
def test_actual_service_executes_all_four_branches_and_hydrates_authority(
    ...
    try:
        ...
        if result.outcome is not RetrievalOutcome.COMPLETE:
            if recording.executions is None:
                pytest.fail(
                    f"adapter failed before branch tuple: {recording.exception!r}",
                    pytrace=True,
                )
            query_digest = recording.executions[0].query_digest
            try:
                ...
                pytest.fail(
                    f"validator rejected live branches: {type(exc).__name__}: {exc}; "
                    f"executions={recording.executions!r}",
                    pytrace=True,
                )
            try:
                ...
                pytest.fail(
                    f"fusion rejected live branches: {type(exc).__name__}: {exc}; "
                    f"executions={recording.executions!r}",
                    pytrace=True,
                )
            pytest.fail(
                "live branches passed validator and fusion but authority still failed: "
                f"reason={None if result.failure is None else result.failure.reason_code}; "
                f"executions={recording.executions!r}; retained={retained!r}; "
