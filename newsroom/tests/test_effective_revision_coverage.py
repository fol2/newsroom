...
def test_delayed_replay_keeps_durable_effective_pull_first_seen() -> None:
    ...
    landed_at = "2026-08-20T09:01:00.000000Z"
    retain_effective_revision_first_seen(
        connection,
        source_id="UK-01",
    ...
    replayed = GroupedObservation(
        "UK-01",
        "sha256:replayed",
        item,
        "2026-08-30T09:01:00.000000Z",
    )

    revision = revisions_from(
        units_from(
            (replayed,),
            proving_run_id="replay",
            effective_revision_resolver=EffectiveRevisionIdentityResolver(connection),
        )
    )[0]
    connection.close()

    assert revision.observed_at == landed_at
