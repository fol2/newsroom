...
def test_grag_053_uses_the_reviewed_package_neutrality_decision() -> None:
    ...
    assert "## package neutrality" in normalized_decision
    assert (
        "must not be equated with mandatory use of microsoft graphrag "
        "community summarisation"
    ) in normalized_decision
    assert "not required runtime dependencies" in normalized_decision
    assert "cannot satisfy a required retrieval mode" in normalized_decision
