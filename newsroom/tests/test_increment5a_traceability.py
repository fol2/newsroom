...
def test_delivery_map_matches_dependency_order_without_overclaim() -> None:
    ...
    for row in INCREMENT_5_TRACEABILITY:
        if row.delivery_trace is Increment5DeliveryTrace.DELIVERED_IN_5A:
            ...
            assert row.delivery_issue in {143, 144}
            assert row.delivery_target in {
                "increment-3-accepted-authority",
...
def test_graphrag_use_cases_and_blockers_use_exact_machine_plan_anchors() -> None:
    rows = _rows()
