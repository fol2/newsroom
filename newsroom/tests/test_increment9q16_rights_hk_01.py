...
_COUNTS = PROBE_COUNTS_BY_GATE[HK_01_GATE_ID]
_UNEMITTED = (
    "RIGHTS_HK-02",
    "RIGHTS_HK-04",
    "RIGHTS_RAD-01",
...
def _inventory_bytes() -> bytes:
    return PACKAGE_FIXTURES_HK_01.joinpath(INVENTORY_NAME).read_bytes()
...
def _authorised_inventory(tmp_path: Path) -> Path:
    root = tmp_path / "fixtures"
    root.mkdir()
    for path in PACKAGE_FIXTURES_HK_01.iterdir():
...
def test_assess_fails_closed_without_inventory(tmp_path: Path) -> None:
    with pytest.raises(QualificationError, match="inventory"):
        assess(tmp_path / "missing", gate=HK_01_GATE_ID)
...
def test_assess_fails_closed_without_a_valid_inventory(tmp_path: Path) -> None:
    ...
    root.joinpath(HMAC_KEY_NAME).write_bytes(FIXTURE_HMAC_KEY)
    for rc in REFUSAL_CLASSES:
        (root / rc).write_bytes(PACKAGE_FIXTURES_HK_01.joinpath(rc).read_bytes())
    with pytest.raises(QualificationError, match="inventory"):
        assess(root, gate=HK_01_GATE_ID)
    with pytest.raises(QualificationError, match="inventory"):
        assess(root, gate=HK_01_GATE_ID, rights_inventory={})
...
def test_news_pool_paths_are_rejected(tmp_path: Path) -> None:
    with pytest.raises(QualificationError, match="news_pool"):
        assess(tmp_path / "news_pool.sqlite3", gate=HK_01_GATE_ID)


def test_assess_emits_qualification_evidence_not_a_gate_record(tmp_path: Path) -> None:
    root = _authorised_inventory(tmp_path)
    evidence = assess(root, gate=HK_01_GATE_ID)
    again = assess(root, gate=HK_01_GATE_ID)
    assert evidence.evidence_digest == again.evidence_digest
    first = assess_rights(
        HK_01_GATE_ID, inventory=fixture_inventory(gate=HK_01_GATE_ID), now=FIXTURE_NOW
    )
    ...
    assert first.endpoint == evidence.endpoint == HK_01_ENDPOINT
    assert first.source_role == evidence.source_role == HK_01_SOURCE_ROLE
    assert first.families == evidence.families == tuple(sorted(FIXTURE_FAMILIES))
    assert len(set(first.families)) == 3
    assert evidence.gate_id == HK_01_GATE_ID == "RIGHTS_HK-01"
    ...
    assert b'"gate_id":"RIGHTS_HK-01"' in payload
    assert b'"unanimous":true' in payload
    assert HK_01_ENDPOINT.encode() in payload
    ...
    assert b"RIGHTS_HK-02" not in payload
    assert b"info.gov.hk" not in payload
    assert b"/en/common/html/topstories.rss.xml" not in payload
...
def test_assess_fails_closed_when_a_probe_mutates(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationError, match="mutated"):
        assess(root, probe=mutate, gate=HK_01_GATE_ID)
...
def test_assess_fails_closed_when_not_all_refusals_engage(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationError, match="not all refusals"):
        assess(root, probe=partial_engage, gate=HK_01_GATE_ID)


def test_authorised_package_fixtures_assess_without_mutating() -> None:
    evidence = assess(PACKAGE_FIXTURES_HK_01, gate=HK_01_GATE_ID)
    assert evidence.status == "PASS"
    assert evidence.refusals_engaged == len(REFUSAL_CLASSES)
    assert all(item.before_digest == item.after_digest for item in evidence.refusals)
    writer = probe_for(HK_01_GATE_ID)
    for rc in REFUSAL_CLASSES:
        assert writer(rc, PACKAGE_FIXTURES_HK_01 / rc) is True
...
def test_ten_refusal_classes_fail_closed_on_the_real_contracts() -> None:
    authorised = fixture_inventory(gate=HK_01_GATE_ID)
    assert assess_rights(HK_01_GATE_ID, inventory=None, now=FIXTURE_NOW).status == "FAIL"
    empty = dict(authorised)
    empty["reviews"] = []
    assert assess_rights(HK_01_GATE_ID, inventory=empty, now=FIXTURE_NOW).status == "FAIL"
    one = dict(authorised)
    one["reviews"] = [fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID)]
    two = dict(authorised)
    two["reviews"] = [
        fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID),
        fixture_review(FIXTURE_FAMILIES[1], gate=HK_01_GATE_ID),
    ]
    for inventory in (one, two):
        assert (
            assess_rights(HK_01_GATE_ID, inventory=inventory, now=FIXTURE_NOW).status
            == "FAIL"
        )
    ...
    duplicate["reviews"] = [
        ...
        fixture_review(
            FIXTURE_FAMILIES[0],
            gate=HK_01_GATE_ID,
            reviewer_id="reviewer-duplicate-9q16",
        ),
    ]
    assert (
        assess_rights(HK_01_GATE_ID, inventory=duplicate, now=FIXTURE_NOW).status
        == "FAIL"
    )
    missing = fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID)
    del missing["retention"]
    extra = fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID)
    extra["extra"] = "field"
    vacant = fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID, destinations=[])
    rest = authorised["reviews"][1:]
    for bad in (missing, extra, vacant):
        broken = dict(authorised)
        broken["reviews"] = [bad, *rest]
        assert (
            assess_rights(HK_01_GATE_ID, inventory=broken, now=FIXTURE_NOW).status
            == "FAIL"
        )
    ...
    assert (
        assess_rights(HK_01_GATE_ID, inventory=sealed, now=FIXTURE_NOW).status == "FAIL"
    )
    for verdict in ("FAIL", "WAIVE"):
        other = dict(authorised)
        other["reviews"] = [
            fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID, verdict=verdict),
            *rest,
        ]
        assert (
            assess_rights(HK_01_GATE_ID, inventory=other, now=FIXTURE_NOW).status
            == "FAIL"
        )
    for changes in (
        {"gate_id": UK_10_GATE_ID},
        {"source_role": BINDINGS[UK_10_GATE_ID][1]},
        {"endpoint": BINDINGS[UK_10_GATE_ID][2]},
    ):
        mismatched = dict(authorised)
        mismatched["reviews"] = [
            fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID, **changes),
            *rest,
        ]
        assert (
            assess_rights(HK_01_GATE_ID, inventory=mismatched, now=FIXTURE_NOW).status
            == "FAIL"
        )
    uk10 = fixture_inventory(gate=UK_10_GATE_ID)
    assert assess_rights(HK_01_GATE_ID, inventory=uk10, now=FIXTURE_NOW).status == "FAIL"
    ...
    assert (
        assess_rights(HK_01_GATE_ID, inventory=drifted, now=FIXTURE_NOW).status == "FAIL"
    )
    expired = dict(authorised)
    expired["reviews"] = [
        fixture_review(FIXTURE_FAMILIES[0], gate=HK_01_GATE_ID, expires_at=FIXTURE_NOW),
        *rest,
    ]
    ...
    assert (
        assess_rights(HK_01_GATE_ID, inventory=expired, now=FIXTURE_NOW).status == "FAIL"
    )
    assert (
        assess_rights(HK_01_GATE_ID, inventory=future, now=FIXTURE_NOW).status == "FAIL"
    )
    ...
    rights_gate = next(g for g in bare if g.gate_id == HK_01_GATE_ID)
    assert rights_gate.status is GateStatus.FAIL
    assert rights_gate.reason == "inventory is required"
    assert assess_rights(HK_01_GATE_ID, inventory=True, now=FIXTURE_NOW).status == "FAIL"
    with pytest.raises(RightsError, match="required_gate_ids"):
        refuse_namesake_satisfaction(required_gate_ids(), gate=HK_01_GATE_ID)
    ...
    assert (
        next(g for g in siblings if g.gate_id == HK_01_GATE_ID).status is GateStatus.FAIL
    )
    ...
    assert next(g for g in cross if g.gate_id == HK_01_GATE_ID).status is GateStatus.FAIL
...
def test_campaign_namesake_list_membership_cannot_pass() -> None:
    ...
    assert HK_01_GATE_ID in required_gate_ids()
    ...
    assert "MISSING_GATE:RIGHTS_HK-01" in findings


def test_package_inventory_matches_fixture_inventory() -> None:
    loaded = json.loads(PACKAGE_FIXTURES_HK_01.joinpath(INVENTORY_NAME).read_bytes())
    assert canonical_json_bytes(loaded) == canonical_json_bytes(
        fixture_inventory(gate=HK_01_GATE_ID)
    )
    ...
    assert PACKAGE_FIXTURES_HK_01.joinpath(HMAC_KEY_NAME).read_bytes() == FIXTURE_HMAC_KEY
    assert bound["now"] == FIXTURE_NOW
    for item in bound["reviews"]:
        assert item["endpoint"] == HK_01_ENDPOINT
        assert item["source_role"] == HK_01_SOURCE_ROLE
        assert item["gate_id"] == HK_01_GATE_ID
        ...
        assert "/en/common/html/topstories.rss.xml" not in item["endpoint"]


def test_bindings_match_od001_and_proving_assess_wires_hk_01_independently() -> None:
    assert BINDINGS[HK_01_GATE_ID][1] == HK_01_SOURCE_ROLE
    assert BINDINGS[HK_01_GATE_ID][2] == HK_01_ENDPOINT == SOURCE_URLS["HK-01"]
    assert HK_01_ENDPOINT == next(
        url for source_id, url in PORTFOLIO if source_id == "HK-01"
    )
    from newsroom.increment9.proving import GateStatus
    from newsroom.increment9.proving import assess as proving_assess
    ...
    assert next(g for g in hk01_only if g.gate_id == GATE_ID).status is GateStatus.FAIL
    assert (
        next(g for g in hk01_only if g.gate_id == UK_02_GATE_ID).status is GateStatus.FAIL
    )
    assert (
        next(g for g in hk01_only if g.gate_id == UK_03_GATE_ID).status is GateStatus.FAIL
    )
    assert (
        next(g for g in hk01_only if g.gate_id == UK_05_GATE_ID).status is GateStatus.FAIL
    )
    assert (
        next(g for g in hk01_only if g.gate_id == UK_10_GATE_ID).status is GateStatus.FAIL
    )
    assert (
        next(g for g in hk01_only if g.gate_id == HK_01_GATE_ID).status is GateStatus.PASS
    )
...
def test_hmac_and_injected_now_are_required_for_pass() -> None:
    inventory = fixture_inventory(gate=HK_01_GATE_ID)
    first = assess_rights(HK_01_GATE_ID, inventory=inventory, now=FIXTURE_NOW)
    second = assess_rights(HK_01_GATE_ID, inventory=inventory, now=FIXTURE_NOW)
    assert first == second
    assert first.status == "PASS"
    from newsroom.increment9.proving import GateStatus
    from newsroom.increment9.proving import assess as proving_assess
    ...
    wired = proving_assess(
        ...
        kill_switch=False,
        no_emergency_stop=True,
        rights_hk_01=inventory,
        now=FIXTURE_NOW,
    )
    assert next(g for g in wired if g.gate_id == HK_01_GATE_ID).status is GateStatus.PASS
    late = assess_rights(
        HK_01_GATE_ID, inventory=inventory, now="2026-08-19T00:00:00.000000Z"
    )
    assert late.status == "FAIL"
    early = assess_rights(
        HK_01_GATE_ID, inventory=inventory, now="2026-08-17T23:59:59.000000Z"
    )
