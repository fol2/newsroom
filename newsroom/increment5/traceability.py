...
that a deferred retriever, tool, hydration path, production-readiness gate,
Operational Profile, or qualification run already exists. Every decision anchor
is assigned explicitly; there is no prefix-derived fallback that can point an
auditor at the wrong field.
"""
...
def validate_increment5_traceability() -> None:
    ...
    expected_counts = {
        Increment5DeliveryTrace.DELIVERED_IN_5A: 13,
        ...
        Increment5DeliveryTrace.DEFERRED_TO_5E: 55,
        Increment5DeliveryTrace.OUTSIDE_INCREMENT_5_ACTIVATION: 1,
        Increment5DeliveryTrace.SATISFIED_BY_PRIOR_INCREMENT: 8,
    }
    actual_counts = {
        delivery: sum(
    ...
    prior_delivery_anchors = {
        ...
        "GRAG-042": (
            "main@d03441ef2fa26b5dc83f65d1797abf2b381d8f1a:"
            "newsroom/projection/discovery_traceability.py"
    ...
    if rows["GRAG-042"].delivery_issue != 143:
        raise RuntimeError("GRAG-042 is not attributed to Increment 3")
    for requirement in (
        "GRAG-030",
        "GRPROD-003",
        "GRPROD-005",
