...
A 5A row binds a requirement to reviewed source. It does not claim that a
later retriever, tool, composer, operational control, or qualification run
already exists.
"""
...
def _rows() -> tuple[Increment5TraceabilityRow, ...]:
    result: list[Increment5TraceabilityRow] = []
...
def _require_anchor(
    ...
    if (
        ...
        raise RuntimeError(f"ownership differs: {requirement}")
...
def validate_increment5_traceability() -> None:
    ...
    expected_counts = {
        ...
        Increment5DeliveryTrace.DEFERRED_TO_5E: 72,
    ...
    for requirement, fragment in {
        ...
        if rows[requirement].decision_anchor != plan + fragment:
