...
_ANCHOR_GROUPS: tuple[tuple[str, frozenset[str]], ...] = (
    ...
    (
        f"{_CONTRACT}#/payload/profile_schema_digests",
        frozenset({"GRPROD-004", "GRPROD-015"}),
    ),
    (
    ...
    (
        "issue:#254:deferred:scope-specific-timing-freshness-retry-capacity-alert-objectives",
        frozenset({"DOPS-002"}),
    ),
    (
...
def _build_anchor_map() -> dict[str, str]:
    ...
    if frozenset(result) != ALL_REQUIREMENTS:
        ...
        raise RuntimeError(
            f"decision anchors differ from accepted inventory: missing={missing}, extra={extra}"
        )
    return result


_ANCHOR_BY_REQUIREMENT = _build_anchor_map()
    ...
    for requirement_id in sorted(ALL_REQUIREMENTS):
        ...
        rows.append(
            Increment5TraceabilityRow(
                ...
                decision_anchor=_ANCHOR_BY_REQUIREMENT[requirement_id],
                decision_trace=(
...
def validate_increment5_traceability() -> None:
    ...
    for requirement in ("DOPS-001", "DOPS-002"):
        row = rows[requirement]
        if (
            row.delivery_trace is not Increment5DeliveryTrace.DEFERRED_TO_5E
            or row.delivery_issue != 254
            or not row.decision_anchor.startswith("issue:#254:deferred:")
        ):
            raise RuntimeError("Operational Profile evidence is overstated in 5A")
