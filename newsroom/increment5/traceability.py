...
that a deferred retriever, tool, hybrid composer, lineage path, hydration path,
production-readiness gate, Operational Profile, or qualification run already
exists. Every decision anchor is assigned explicitly; there is no prefix-derived
fallback that can point an auditor at the wrong field.
"""
...
def validate_increment5_traceability() -> None:
    ...
    for requirement, expected_anchor in evaluation_plan_anchors.items():
        if rows[requirement].decision_anchor != expected_anchor:
            raise RuntimeError(
                f"GraphRAG qualification plan anchor differs: {requirement}"
            )

    package_neutrality_anchor = (
        "docs/decisions/2026-08-02-increment-5a-production-retrieval-contract.md"
        "#package-neutrality"
    )
    package_neutrality = rows["GRAG-053"]
    if (
        package_neutrality.decision_trace
        is not Increment5DecisionTrace.BOUND_BY_5A
        or package_neutrality.delivery_trace
        is not Increment5DeliveryTrace.DELIVERED_IN_5A
        or package_neutrality.delivery_issue != 250
        or package_neutrality.decision_anchor != package_neutrality_anchor
    ):
        raise RuntimeError("package neutrality is not bound to its reviewed decision")
    if rows["GRPROD-012"].decision_anchor != (
        "newsroom/increment5/data/increment5a_retrieval_contract_v1.json"
        "#/payload/change_control"
    ):
        raise RuntimeError("material change control has the wrong contract anchor")

    prior_delivery_anchors = {
        "GRAG-030": (
    ...
    if frozenset(prior_delivery_anchors) != DELIVERY_GROUPS[
        Increment5DeliveryTrace.SATISFIED_BY_PRIOR_INCREMENT
    ]:
        raise RuntimeError(
            "explicit prior anchors differ from prior delivery inventory"
        )
    for requirement, expected_anchor in prior_delivery_anchors.items():
        row = rows[requirement]
        if (
            row.decision_trace
            is not Increment5DecisionTrace.INHERITED_ACCEPTED_AUTHORITY
            or row.delivery_trace
            is not Increment5DeliveryTrace.SATISFIED_BY_PRIOR_INCREMENT
    ...
    for requirement in (
        ...
        if rows[requirement].delivery_issue != 144:
            raise RuntimeError(
                f"existing graph delivery is not attributed to Increment 4: "
                f"{requirement}"
            )

    deferred_hybrid_anchors = {
        "GRAG-031": (
    ...
    for requirement, expected_anchor in deferred_hybrid_anchors.items():
        row = rows[requirement]
        if (
            row.decision_trace is not Increment5DecisionTrace.BOUND_BY_5A
            or row.delivery_trace is not Increment5DeliveryTrace.DEFERRED_TO_5D
            or row.delivery_issue != 253
            or row.decision_anchor != expected_anchor
        ):
            raise RuntimeError(
                f"hybrid composition or lineage has the wrong owner: {requirement}"
            )
    ...
    challenger = rows["GRAG-051"]
    if (
        challenger.delivery_trace is not Increment5DeliveryTrace.DEFERRED_TO_5E
        or challenger.delivery_issue != 254
        or challenger.decision_anchor != challenger_anchor
    ):
        raise RuntimeError("conditional challenger policy is claimed too early")

    deferred_production_anchors = {
        "GRPROD-004": (
    ...
    for requirement, expected_anchor in deferred_production_anchors.items():
        row = rows[requirement]
        if (
            ...
            raise RuntimeError(
                f"production readiness is overstated or misplaced: {requirement}"
            )
