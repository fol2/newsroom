...
INCREMENT8_FINAL_NON_EFFECTS = tuple(
    sorted(
        {
...
def _case(
    case_id: str, lane: Increment8CloseoutLane, module: str, test: str, requirement: str
) -> Increment8CloseoutCase:
    return Increment8CloseoutCase(
        case_id, lane, f"newsroom.tests.{module}::{test}", requirement
    )


_D = Increment8CloseoutLane.DETERMINISTIC
_S = Increment8CloseoutLane.ACTUAL_NEO4J
INCREMENT8F_FINAL_CLOSEOUT_CASES = tuple(
    sorted(
        (
            _case(
                "A01_EVALUATION_RELEASE",
                _D,
                "test_increment8a_evaluation_authority",
                "test_pass_requires_full_exposure_primary_labels_and_second_review_sample",
                "EVALUATION_RELEASE",
            ),
            _case(
                "B01_METRICS",
                _D,
                "test_increment8b_metrics",
                "test_complete_report_is_reconstructed_from_frozen_context_and_reviewed_cases",
                "PROSPECTIVE_METRICS",
            ),
            _case(
                "C01_OPERATIONS",
                _D,
                "test_increment8c_operations",
                "test_capacity_evidence_covers_all_scenarios_and_frozen_limits",
                "OPERATIONS_CAPACITY",
            ),
            _case(
                "C02_HANDOFF",
                _D,
                "test_increment8c_operations",
                "test_self_consistent_anchor_rewrite_fails_against_pinned_registration_digest",
                "HANDOFF_REGISTRATION_ANCHOR",
            ),
            _case(
                "D01_HEALTH_SECURITY",
                _D,
                "test_increment8d_observability",
                "test_security_admission_is_exact_versioned_and_never_activation",
                "HEALTH_SECURITY",
            ),
            _case(
                "D02_INCIDENT",
                _D,
                "test_increment8d_observability",
                "test_incident_lifecycle_is_append_only_and_integrity_close_requires_regression_case",
                "OBSERVABILITY_INCIDENT",
            ),
            _case(
                "E01_MIGRATION",
                _D,
                "test_increment8e_recovery",
                "test_v31_to_v32_requires_exact_backup_and_preserves_prefix",
                "RECOVERY_MIGRATION",
            ),
            _case(
                "E02_BACKUP_RESTORE",
                _D,
                "test_increment8e_recovery",
                "test_checked_backup_restore_preserves_authority_and_requires_reconciliation",
                "BACKUP_RESTORE",
            ),
            _case(
                "E03_FAULT_RECOVERY",
                _D,
                "test_increment8e_recovery",
                "test_fault_injection_is_fixture_only_and_verifies_fail_closed_outcomes",
                "FAULT_PURGE_REPLAY",
            ),
            _case(
                "F01_HARDWARE_COST",
                _D,
                "test_increment8f_admission",
                "test_hardware_cost_and_licence_values_are_exact_and_non_activating",
                "HARDWARE_COST_LICENCE",
            ),
            _case(
                "F02_ADMISSION",
                _D,
                "test_increment8f_admission",
                "test_complete_packet_binds_every_gate_and_admits_only_fixture_operation",
                "OPERATIONAL_ADMISSION",
            ),
            _case(
                "G01_INVENTORY",
                _D,
                "test_increment8f_closeout_receipt",
                "test_increment8_closeout_inventory_and_contract_are_exact",
                "EXACT_MAIN_RECEIPT",
            ),
            _case(
...
INCREMENT8_FINAL_CLOSEOUT_INVENTORY_DIGEST = digest_bytes(
    canonical_json_bytes(_values())
)
...
def increment8_final_migration_history(
    ...
    if (
        ...
        != tuple(range(1, INCREMENT8_FINAL_SCHEMA_VERSION + 1))
...
def validate_increment8_closeout_inventory() -> None:
    ...
    if INCREMENT8_FINAL_CLOSEOUT_INVENTORY_DIGEST != digest_bytes(
        canonical_json_bytes(_values())
    ):
