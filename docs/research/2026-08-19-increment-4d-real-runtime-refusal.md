...
| Hydrate stored real row | `_graphiti_adapter_store_common._graphiti_configuration_from_row` | Fake and replay reconstruct from closed helpers. Any other mode raises `AuthorityPersistenceError("real Graphiti configuration is disabled and unqualified")` — a real row is never hydrated. |
| Attempt re-validation | `_graphiti_adapter_store_common` | Calls `configuration.require_execution_authorized()` on the reconstituted configuration (fake/replay only, because real rows never hydrate). |
| Authorised workspace set | `_graphiti_adapter_store_common._graphiti_workspace_policy_from_row` | Only the two seeded `DENY_ALL` / `NONE` policies (qualification and replay) are in the closed set. A real `APPROVED_PROVIDER_ONLY` policy is `outside the authorised closed set`. Schema v16 seeds no real-runtime workspace policy. |
