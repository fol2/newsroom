...
- **Machine-record digest:** `sha256:f7e8cc0a74cff01b76655997b45730a659c92a8d3e9e9c777d0cee74b281263c`
...
Calibration and qualification remain disjoint. Calendar duration alone is not sufficient exposure. The exact machine-plan floors—100 unique qualification cases, family minima of 30/30/40, at least 10 cases per required case type and at least 20 relevant cases per required slice—must all be met before the affected result can be evaluated. Any shortfall is `NOT_EVALUATED`, never passed. The reviewed signed dataset is a bounded evaluation universe; no provider, index or retrieval branch is treated as complete ground truth.
