...
`NOT_EVALUATED`. Profile-validation inner receipt v6 authenticates neither its
executed source nor its runtime closure and is never admissible alone.

Before execution, the signed exact-head workflow freezes and binds the exact
validator byte buffer and digest, child command, runner image, Python executable,
loaded standard-library closure or approved immutable runtime identity, reviewed
commit/tree, manifest and Epoch. It writes the same hashed byte buffer directly
to child stdin, then binds the canonical inner-receipt digest in an outer signed
evidence envelope.

A source buffer mismatch, mutable or unbound intermediate pipe, missing runtime
closure identity, different inner receipt, absent outer envelope, or code-tree
mismatch is `NOT_EVALUATED`. Superseded Epoch Runs remain retained. The Epoch
