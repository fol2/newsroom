...
The product layer does not call GitHub, inspect a PR, authenticate comments, pin its own import graph, mint a capability or require a second post-merge materialisation record. Digests identify content; they are not a competing authorization system. A material contract change requires a new contract version and a new reviewed merge.

A fresh isolated process used to inspect canonical profile bytes is not an admission or authorization subsystem. Its receipt has effect `NONE`; it merely proves that exact bytes passed the reviewed profile structure and semantic checks in that process.
...
Standalone JSON-Schema consumers therefore receive reviewed identity checks. The reviewed-binding schema digests are source-governed by PR #255 but are intentionally not inserted back into the contract they bind; doing so would create a schema-digest/contract-digest cycle.

### Profile validation authority boundary

Python code in one process is not a security principal against other arbitrary code in that same process: closures, globals, function code and return objects are mutable. 5A therefore exposes no public `ValidatedProfileManifest`, no public in-process validation certificate and no boolean result that can grant qualification authority.

The public Python builders are deterministic conveniences. Their private check can detect ordinary malformed or incompatible manifests, but its return value is always `None` and is not admissible evidence. A caller can fabricate a dictionary regardless of any builder; no dictionary or in-process object authorizes qualification.

For 5E evidence, the exact canonical manifest bytes must be supplied to:

```text
python -I scripts/sdlc/increment5_profile_validator.py
```

inside the fresh exact-head signed workflow process. The validator rejects non-canonical JSON, duplicate names, unreviewed contract/component identities, wrong profile/eligibility pairs, changed budgets/effects, fixture substitution, unsafe dataset state and missing actual-service requirements. It emits a canonical receipt bound to the manifest digest and profile kind with:

- `authority_effect = NONE`;
- `qualification_authority_granted = false`; and
- `production_activation_authorized = false`.

That receipt is necessary profile-validation evidence for 5E, but it is never sufficient qualification evidence and grants no component, source, model, provider, spend, write, production or public-effect authority. The actual 5E run must separately satisfy all frozen execution, service, data, rights, security and evaluation gates.
...
- **5E / #254:** fresh-process profile validation, hybrid-target qualification, comparative ablations, conditional challenger policy, production-readiness validation, Operational Profiles, per-family blocking criteria, zero-tolerance temporal/rebuild gates, security, purge, recovery and actual-Neo4j qualification.
