...
only `None` and is not admissible evidence. The isolated validator likewise
cannot authenticate its own executable bytes or the integrity of the Python
standard-library code that already loaded it. 5A therefore forbids the inner
process from claiming either source or runtime attestation.

The validator may perform deterministic profile structure and semantic checks,
read bounded reviewed Git objects, and return a canonical **inner receipt v6**.
That receipt is deliberately fail-closed:
...
- `outer_runtime_binding_required = true`; and
- `validation_evidence_admissible = false`.

Names such as the reviewed commit, tree, contract, schema, expected validator
blob and observed Python/Git paths or digests identify what the inner process
inspected. They do not prove which source bytes executed or that the runtime
closure was trustworthy. The inner receipt grants no qualification, component,
production, source, model, provider, spend, write, publication or public-effect
authority.

Only the fresh signed exact-head 5E workflow may create an admissible outer
evidence envelope. Before child execution it must bind one exact byte buffer for
the validator, hash that same buffer, write that same buffer to the child stdin
without an intervening mutable pipe, and retain the child command and byte
digest. It must independently bind the exact runner image, Python executable
and loaded standard-library closure or another owner-approved immutable runtime
identity. The outer envelope then binds those source/runtime facts, the exact
Epoch, the manifest digest and the canonical inner-receipt digest.

A missing outer envelope, a different executed-source digest, an unbound runtime
closure, or any mismatch is `NOT_EVALUATED`. A self-reported v6 inner receipt is
never qualification evidence by itself. This separation is intentional: source
governance and signed workflow evidence authenticate execution; application
code does not recursively admit itself.
