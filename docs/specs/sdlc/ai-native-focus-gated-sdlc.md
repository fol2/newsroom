...
- expected ordinary job/bootstrap counts.
...
| `evidence.yml` | schedule, merge group, manual | full deterministic repository health |
...
An ordinary documentation PR starts one job and zero dependency bootstraps. An ordinary executable PR starts one job and one locked bootstrap. Obsolete heads are cancelled. Scheduled/manual full health and research remain outside the ordinary critical path.
