...
A requirement bound by 5A is not reported as an implemented retriever, hybrid composer, tool, lineage path, hydration path, production-readiness gate, Operational Profile or qualification run.
...
|---|---:|---|
| 5A / #250 | 12 | Contract, reviewed-bound profile schemas, frozen plan, authority/non-effect/package-neutrality decisions and traceability |
| 5B / #251 | 1 | Four independent typed retriever implementations and branch receipts |
| 5C / #252 | 6 | Six bounded named read-only tools |
| 5D / #253 | 31 | Hybrid fusion/deduplication, complete discovery lineage, authoritative hydration, freshness, reconciliation and degradation |
| 5E / #254 | 56 | Conditional challenger policy, production-readiness validation, Operational Profiles/admission, retrieval qualification, security, purge, recovery and decision evidence |
...
`DEVAL-010`, `DEVAL-011`, `DEVAL-012`, `DEVAL-051`, `DEVAL-072`, `DOPS-076`, `GRAG-052`, `GRAG-053`, `GRAG-058`, `GRPROD-002`, `GRPROD-023`, `GRPROD-032`.

These rows are verified only by the 5A contract/profile/plan/traceability tests and reviewed documents.

## Package neutrality

`GRAG-053` is not derived from generic material-change control. It points to the exact reviewed `#package-neutrality` section in the 5A decision record.

That decision states that GraphRAG is the repository architecture and semantic boundary, not a mandatory dependency on Microsoft GraphRAG community summarisation. Microsoft GraphRAG, its community-summarisation pipeline and equivalent third-party packages are optional implementation choices rather than required modes or authority surfaces. Installing such a package cannot satisfy a required branch, create authority, replace governed hydration or alter rights, budgets, security, temporal or failure rules.

`GRPROD-012` remains separately anchored to the machine contract’s material-change control. The two requirements cannot be collapsed back into one generic pointer.

## Exact prior delivery

Only seven rows are satisfied by prior increments, all from accepted Increment 4:
...
Those rows point to exact entries in `newsroom/increment4/traceability.py` at accepted `main@c9e31879421083e82e2538d57087d04e9b454d34`, delivery issue #144.
...
Increment 3 remains an important graph foundation, but it is not credited with `GRAG-042`. Its accepted evidence explicitly excluded `EVENT_HYPOTHESIS_CANDIDATE_OR_EVIDENCE_HANDOFF`, and its mapping did not contain an Event Hypothesis node. It therefore did not deliver the required projectable `Source → Revision → Signal → Lead → Hypothesis → Candidate` lineage.

## Hybrid composition and complete lineage

`GRAG-031` is assigned to 5D/#253, not 5B. Issue #251 ends with four independently testable branches and exact branch receipts. The hybrid system exists only after deterministic reciprocal-rank fusion and authoritative dependency-root deduplication combine those branches while retaining every origin and exclusion receipt.

`GRAG-042` is also assigned to 5D/#253. 5D must make the complete six-stage discovery lineage projectable and hydratable, with unverified states explicit, before 5E can qualify timeline, provenance, source-impact or false-merge behaviour.

The machine map pins both requirements to explicit #253 anchors and tests their decision state, delivery state, issue and target. Neither can regress to an independent-branch issue or an incomplete prior trace.

## Challenger, production and operational evidence remains deferred

`GRAG-051` remains in 5E/#254. A challenger engine may be tested only after a measured blocker against the selected target or a retained owner-approved comparison purpose. Multiple engines are not implemented in parallel by default. The generic material-change rule does not satisfy this conditional policy.
...
- `GRAG-031` → explicit #253 hybrid fusion and dependency-root deduplication ownership;
- `GRAG-042` → explicit #253 complete Source-to-Candidate lineage projection and hydration ownership;
- `GRAG-051` → explicit #254 measured-blocker or owner-approved-comparison challenger gate;
- `GRAG-053` → the reviewed 5A decision’s exact package-neutrality section;
...
- package neutrality being anchored to generic change control or a document without the exact Microsoft GraphRAG community-summarisation rule;
...
- independent branch delivery being reported as a completed hybrid system;
- incomplete Source-to-Candidate lineage being reported as prior `GRAG-042` delivery;
- a challenger being admitted without a measured blocker or owner-approved comparison purpose;
