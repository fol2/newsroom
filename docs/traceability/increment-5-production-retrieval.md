...
The dependency split follows two categorical boundaries:

- **5C ends at named-tool mechanics.** It owns six typed bounded read-only tools,
  their local caller/purpose/scope checks, bounded request shapes, and explicit
  tool outcomes. This is necessary input validation, but it is not the complete
  operational `DOPS-026` boundary.
...
- **5E owns system operation.** It owns Operational Profiles, the complete
  source/model-content policy boundary, health and coverage posture, queue
  policy, durable transition delivery, mandatory later reconciliation,
  containment, dependency-specific failures, security, recovery, and
  qualification.

This prevents local input validation or a request-local `DEGRADED` outcome from
being mistaken for operational policy, egress, budget, authority, or recovery
control.
...
| 5E / #254 | 73 | operational policy, admission, health, queues, durability, reconciliation, containment, security, recovery, and qualification |
...
egress rule, budget, or authority surface.

`DOPS-026` therefore belongs to 5E. Its complete evidence must cover all
executable operational surfaces, including source/model content, policy,
egress, budgets, credentials, destinations, and authority—not merely the local
named-tool parser or authorization gate.
...
it states that admission is not activation. Every other accepted `DOPS-*` row
belongs to 5E.
...
- `DOPS-067` → 5E/#254: tool-local authorization does not prove least-privilege credentials, source-access scopes, and approved destinations.
...
counts; deferred rows without the exact issue; any `DOPS-*` row in 5C or 5D;
any accepted operational `DOPS-*` row other than `DOPS-076` assigned before 5E;
`TRI-028` assigned before durable later reconciliation; material
