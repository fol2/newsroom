...
| Increment 4 / #144 | 8 | Already accepted authority/projection foundations |
...
`GRPROD-003`, `GRPROD-013`, `GRPROD-014` and `GRPROD-020` are not credited to 5A. Increment 4 already delivered and tested:

- repository-owned ontology, mapping, controller, operations and tests;
- engine-neutral canonical semantics independent of Neo4j internal and Graphiti-private identity;
- the versioned repository ontology/mapping/controller/actual-service workflow definition; and
- the first ontology/mapping/graph-boundary/health/integration proof beside relational authority.

Their rows are `INHERITED_ACCEPTED_AUTHORITY`, `SATISFIED_BY_PRIOR_INCREMENT`, delivery issue #144, with exact requirement anchors into `newsroom/increment4/traceability.py`.

The other prior-delivery rows remain `GRAG-030`, `GRAG-042`, `GRPROD-005` and `DOPS-007`, making eight prior-delivery rows in total.
...
- an accepted Increment 4 implementation being falsely credited to 5A;
