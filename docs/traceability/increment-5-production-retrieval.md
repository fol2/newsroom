...
|---|---:|---|
| 5A / #250 | 16 | Contract, reviewed-bound profile schemas, frozen plan, authority/non-effect decisions and traceability |
...
| Increment 4 / #144 | 6 | Already accepted authority/projection foundations |
...
`DEVAL-010`, `DEVAL-011`, `DEVAL-012`, `DEVAL-051`, `DEVAL-072`, `DOPS-070`, `DOPS-076`, `GRAG-051`, `GRAG-052`, `GRAG-053`, `GRAG-058`, `GRPROD-002`, `GRPROD-003`, `GRPROD-013`, `GRPROD-023`, `GRPROD-032`.

These rows are verified only by the 5A contract/profile/plan/traceability tests and reviewed documents.

## Existing Increment 4 delivery

`GRPROD-014` and `GRPROD-020` are not credited to 5A. Increment 4 already delivered and tested the versioned repository ontology/mapping/controller/actual-service workflow definition and the first ontology/mapping/graph-boundary/health/integration proof beside relational authority. Their rows are `INHERITED_ACCEPTED_AUTHORITY`, `SATISFIED_BY_PRIOR_INCREMENT`, delivery issue #144, with exact anchors into `newsroom/increment4/traceability.py`.

The other prior rows remain `GRAG-030`, `GRAG-042`, `GRPROD-005` and `DOPS-007`, making six prior-delivery rows in total.
...
- `GRPROD-014` / `GRPROD-020` → exact accepted Increment 4 implementation and test evidence;
