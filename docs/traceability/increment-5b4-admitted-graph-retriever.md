...
tool, complete hydration or `DOPS-*` operational admission. Those remain 5C and 5D work; operational admission remains in
Increment 8 / issue #148.

## Completion evidence

Issue #308 is a Tier-S service atom. It can close only after one clean canonical
product commit over the exact integrated `main` has:

- focused and complete deterministic CI passing;
- authenticated actual-Neo4j mandatory tests passing with zero required skips;
- the affected Authority and Projection lanes passing;
- current-head substantive review with zero unresolved P1/material-P2 findings;
- zero unresolved review threads; and
- product-only squash merge with exact commit/tree evidence.

The all-applicable permanent-workflow and signed-SDLC gate runs once at parent
#251 as the Tier-M integrated 5B closeout; it is not repeated as a 5B4 leaf gate.
