...
| `GRAG-035` retained composition-evidence integrity | exact integer bounds/counters plus mode-specific passage/trust and closed admitted graph-path semantics | float/boolean scalar substitution, rank confusion, path direction/predicate/self-loop and origin-mode tamper regressions |
