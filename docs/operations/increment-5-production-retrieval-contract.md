...
Every 5E evidence manifest is a bounded canonical file. The isolated
validator returns only inner receipt v6: deterministic structure and semantic
results plus reviewed/observed identities. It explicitly sets source and runtime
attestation to false, requires both outer bindings, and marks itself
`validation_evidence_admissible=false`.

The signed exact-head workflow—not the child process—owns executable-source and
runtime evidence. It hashes the exact validator byte buffer that it directly
supplies to the child; retains the exact command; independently binds the runner
image, Python executable and loaded standard-library closure or another approved
immutable runtime identity; and wraps the canonical inner receipt in an outer
Epoch-bound evidence envelope. No shell pipeline whose intermediate bytes are
unbound may satisfy this rule.

Without that outer envelope, or when any source/runtime/inner-receipt/Epoch
digest differs, the result is `NOT_EVALUATED`. Neither inner nor outer profile
validation receipt grants qualification or activation authority by itself.
