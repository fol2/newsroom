...
## Profile-validation process boundary

The public Python package exposes profile builders and reviewed schema locations. It does not export `ValidatedProfileManifest`, `validate_profile_manifest`, an eligibility certificate or another in-process authority object. Arbitrary same-process code can mutate Python closures, globals, function code and objects; those mechanisms are not security boundaries.

The builders run a private deterministic check for ordinary developer feedback, but that helper always returns `None`. Its success cannot be retained as qualification evidence.

Every profile manifest used by 5E must be canonical JSON and must be validated in the fresh exact-head workflow process with:

```text
python -I scripts/sdlc/increment5_profile_validator.py
```

The process reads at most 1,048,576 bytes from standard input; rejects invalid UTF-8, duplicate object names, non-canonical JSON, extra/missing fields, identity drift, wrong profile/eligibility pairs, widened effects/budgets, fixture substitution, unsafe datasets and missing actual-service controls; and emits one canonical receipt bound to the manifest digest.

The receipt is deliberately non-authoritative:

- `authority_effect = NONE`;
- `qualification_authority_granted = false`;
- `production_activation_authorized = false`; and
- validation scope is only reviewed profile structure and semantics.

5E retains the exact input bytes, manifest digest, validator receipt, exact head/tree and signed workflow identity. It must still prove actual-service execution, rights, security, exposure, quality, temporal, rebuild and operational gates. A profile-validation receipt alone never qualifies a system.
