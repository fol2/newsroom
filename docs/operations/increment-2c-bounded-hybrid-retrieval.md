...
The authority boundary revalidates exact query identities, branch order, result bounds, trust scope, source identity, branch-specific score domains and mandatory prior-candidate coverage. Exact and direct admitted-relation fixture scores must be one; vector scores remain within `[0, 1]`; full-text scores are finite and non-negative. A substituted typed adapter cannot omit the admitted relation, relabel a query or forge score evidence and still create context authority.
