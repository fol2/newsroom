...
Repository boundary tests share one immutable production-import inventory only when the exact relative paths and source bytes are identical. Any byte or path change creates a different cache key and forces a new AST parse. Watchdog and subprocess timeout fixtures use explicit readiness evidence and shorter waits while retaining the same shared-deadline, process-group termination, child `SIGTERM`, output truncation and unauthorized-background-process assertions.

The complete deterministic pytest inventory runs in exactly four server-owned shards. The lane discovers every regular `test_*.py` file beneath `newsroom/tests`, assigns each file exactly once with a stable size-balanced algorithm, disables pytest's shared cache provider, and gives every shard a private base temporary directory. Callers cannot choose the shard count, file membership, report paths or test exclusions. All shard processes remain inside the one outer lane process group and the unchanged `55`-second deadline. Their logs are replayed in fixed shard order, their reports are independently parsed for duplicate identities and unsafe evidence, and they are merged into the existing single authoritative `pytest.xml`. The merged report must preserve the exact test-identity digest and all pass, failure, error, skip, duration and first-failure evidence before shard reports are removed. A missing report, duplicate test, failed shard or mismatched merge fails closed.

These changes are test/evidence infrastructure only and create no runtime product authority.
