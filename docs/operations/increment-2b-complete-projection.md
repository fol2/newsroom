...
- the deterministic core JUnit manifest marks the exact authenticated Neo4j cases optional only in that no-service lane; the service route still selects the complete 2B service file, fixes `NEWSROOM_NEO4J_COMPLETE_SERVICE_REQUIRED=1`, and proves all eight cases execute without skip, failure or error.
